...
HBITMAP CardImage::cardFrontArray[51];
HBITMAP CardImage::cardBackArray[1];

int CardImage::getCardFront(char s, int r, HINSTANCE hInst)
{
...
{
	if (cardBackArray[0] == NULL)
	{
		const wchar_t* frontFileName = L"cardImages/back.bmp";
		HBITMAP back = (HBITMAP)LoadImage(hInst, frontFileName, IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);
		cardBackArray[0] = back;
		return cardBackArray[0];
	}
	else
	{
		return cardBackArray[0];
